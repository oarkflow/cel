...
// executeChain executes a method chain using dynamic pipeline approach
func executeChain(chain *ChainInfo, ctx *Context) (Value, bool) {
	// Get base object
	...
	// Execute chain dynamically - each method operates on the result of the previous
	current := baseObj
	for _, step := range chain.Methods {
		// Evaluate arguments for this step
		...
		// Execute method on current value
		result, err := callMethod(current, step.Method, args)
		if err != nil {
			return nil, false
		}
		current = result
...
func (m *MethodCall) Evaluate(ctx *Context) (Value, error) {
	...
	if chain := detectMethodChain(m); chain != nil {
		if result, ok := executeChain(chain, ctx); ok {
			return result, nil
		}
	}
